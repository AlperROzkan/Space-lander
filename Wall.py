...
# Classe qui décrit un sol a laide de pygame.draw.lines
class Wall:
    ...
    def genere_points(self, pas):
        ...
        while self.nb_plateforme < 2:
            ...
            while largeur_restante < largeur:
                ...
                hauteur_sol = randint(hauteur / 2, hauteur)
                self.liste_points.append([largeur_restante, hauteur_sol])
                ...
                if proba_terrain_plat < 2 and self.nb_plateforme < 3:
                    ...
                    self.liste_points.append([largeur_restante, hauteur_sol])
                    self.liste_atterissage.append([largeur_restante, hauteur_sol])
                    self.nb_plateforme = self.nb_plateforme + 1
        # Retour de la liste
        return self.liste_points

    # Methode permettant de generer les sols avec les points donnés par la méthode générant aléatoirement les points
    # liste_points : liste de points a lier
    def draw_wall(self, liste_points):
        for i in range(0, len(liste_points) - 1):
            self.liste_sols.append(pygame.draw.line(self.window, white, (liste_points[i][0], liste_points[i][1]),
                                                    (liste_points[i + 1][0], liste_points[i + 1][1]), 1))

        for j in range(0, len(self.liste_atterissage)):
            pygame.draw.line(self.window, blue,
                             (self.liste_atterissage[j][0] - self.largeur_atterissage, self.liste_atterissage[j][1]),
                             (self.liste_atterissage[j][0], self.liste_atterissage[j][1]), 3)
